...
class CheckAnyErrorFromOfferSheet:
    ...
    def _check_missing_columns(self):
        if self.type_table == "msp_offers":
            for columns in self.sheet.required_headers['msp_offers_general']:
                    self._check_required_headers(columns)
            if not any(columns in self.headers for columns in self.sheet.required_headers['msp_offers_offered_price']):
                self._check_required_headers(columns)
            if not any(columns in self.headers for columns in self.sheet.required_headers['msp_offers_IES_discount']):
                self._check_required_headers(columns)
        else:
            raise Exception ("A planilha não é do tipo MSP.")
    ...
    def generate_error_missing_columns(self):
        ...
        if self.missing_column > 0:
            ...
            raise Exception(f"As seguintes colunas estão faltando na planilha: {missing_cols}")
    
    def _check_lines(row):
        return row["Mensalidade sem desconto"] == row['Mensalidade com desconto'] * row["Porcentagem de desconto da bolsa (Fixo/1 º Semestre)"]
    
    def check_discount_n_price(self):
            for columns in self.sheet.required_headers['msp_offers_offered_price']:
                if all(header in self.headers for header in columns) and "Mensalidade sem desconto" in self.headers:
